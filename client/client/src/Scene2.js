...
export class Scene2 extends Phaser.Scene {
    ...
    getPlayerName(onlineplayers,sessionID) {
        setTimeout(() => {
        
            console.log("fetching username from " + sessionID)
            ...
            .then((response) => {
              ...
              .then((myJson) => {
                console.log(myJson);
                usernames[sessionID]=myJson['username'];
                console.log("storing" + sessionID + "//" + myJson['username']);
    
                console.log("curret player size:"+ Object.keys(usernames).length)
                onlineplayers.playerNickname.setText(myJson['username']);
                return   usernames[sessionID]
              });
      ...
      getMyPlayerName(sessionID) {
        ...
        .then((response) => {
          ...
          .then((myJson) => {
            console.log(myJson);
            usernames[sessionID]=myJson['username'];
            this.player.playerNickname.setText(myJson['username']);
            return   usernames[sessionID]
          });
        ...
        room.then((room) => room.onMessage((data) => {
                    ...
                    Object.keys(data.players).forEach(playerId => {
                        ...
                        if (playerId !== room.sessionId) {
                            ...
                            onlinePlayers[player.sessionId] = new OnlinePlayer({
                                ...
                                x: player.x,
                                y: player.y,
                                username: this.getPlayerName(onlinePlayers[player.sessionId],player.sessionId)
                            });
                            onlinePlayers[player.sessionId].updateName(this.getPlayerName(onlinePlayers[player.sessionId],player.sessionId))

                        }
                        else {
                            console.log("this is me");
                            this.player.playerNickname.setText(this.getMyPlayerName(room.sessionId));
                        }
                    })
                }
                if (data.event === 'PLAYER_JOINED') {
                    ...
                    if (!onlinePlayers[data.sessionId]) {
                        ...
                        onlinePlayers[data.sessionId].updateName(this.getPlayerName(onlinePlayers[data.sessionId],data.sessionId))
                ...
                if (data.event === 'PLAYER_MOVED') {
                    ...
                    if (this.mapName === onlinePlayers[data.sessionId].map) {
                        ...
                        onlinePlayers[data.sessionId].isWalking(data.position, data.x, data.y,usernames[data.sessionId]);
                    }
                }
